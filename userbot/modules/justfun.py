...
@bot.on(trans_cmd(outgoing=True, pattern=r"xogame(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    botusername = "@xobot"
...
@bot.on(trans_cmd(outgoing=True, pattern=r"wp(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(trans_cmd(outgoing=True, pattern=r"mod(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        ...
        \n  •  **Example  : **{cmd}wp aku syng kmu @RYUUSHINNI\
    "
    }
)
