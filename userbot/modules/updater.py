...
async def print_changelogs(xx, ac_br, changelog):
    changelog_str = (
        f"**⚠︎ Tersedia Pembaruan Untuk [{ac_br}] :\n\n✥ Pembaruan:**\n`{changelog}`"
    )
...
async def upstream(event):
    ...
    if conf == "deploy":
        await xx.edit("`[HEROKU]: Update Deploy TRANS-BOT Sedang Dalam Proses...`")
        await deploy(xx, repo, ups_rem, ac_br, txt)
        return

    if changelog == "" and not force_update:
        await edit_delete(xx, "**⚠︎ TRANS-Userbot Sudah Versi Terbaru ⚠︎**")
        return repo.__del__()

    if conf == "" and not force_update:
        await print_changelogs(xx, ac_br, changelog)
        await event.delete()
        return await xx.respond(
