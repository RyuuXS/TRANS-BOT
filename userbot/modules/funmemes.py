...
@bot.on(trans_cmd(outgoing=True, pattern=r"gangstar(?: |$)(.*)"))
async def _(event):
...
@bot.on(trans_cmd(outgoing=True, pattern=r"plane(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
@bot.on(trans_cmd(outgoing=True, pattern=r"dump(?: |$)(.*)"))
async def _(message):
    try:
...
@bot.on(trans_cmd(outgoing=True, pattern=r"kiss(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    deq = deque(list("😗😙😚😚😘"))
